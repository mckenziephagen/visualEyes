...
def dataframe_validation(df, screen_dimensions=None, drop_outlier=False, drop_nan=True):
    ...
    if set(['axp', 'ayp']).issubset(df.columns):
        ...
        # Drop NaN values
        if drop_nan:
            df = df.dropna(subset=['axp', 'ayp'])
            
        # Assign x and y coordinates
        x_coord, y_coord = df['axp'], df['ayp']
        ...
        # Assign x and y coordinates
        x_coord, y_coord = df['xpos'], df['ypos']
    ...
    if screen_dimensions:
        ...
        if drop_outlier:
            ...
            x_coord = df['axp'] if 'axp' in df.columns else df['xpos']
            y_coord = df['ayp'] if 'ayp' in df.columns else df['ypos']
