...
def plot_as_scatter(
    data, screen_dimensions, aoi_definitions=None, save_png=None, save_path=None
):
    """
    ...
    out_of_bounds_x = np.where((x_coord < 0) | (x_coord >= screen_dimensions[1]))[
        0
    ]  # list of indices
    out_of_bounds_y = np.where((y_coord < 0) | (y_coord >= screen_dimensions[0]))[
        0
    ]  # list of indices
    ...
    if "axp" in clean_data.columns and "ayp" in clean_data.columns:
        ...
        ax.scatter(
            ...
            marker="o",
            facecolors="none",
            s=[160 * mag for mag in mag_factor],
        )
    else:
        ax.scatter(
            clean_data["xpos"], clean_data["ypos"], color="skyblue", marker="o", s=60
        )
...
def aoi_overlay(aoi_definitions, screen_width, screen_height):
    ...
    for idx, aoi in enumerate(
        aoi_definitions
    ):  # check for each AOI and make the error specific to that AOI
        ...
        if shape not in ["rectangle", "circle"]:
            raise ValueError(
                f"Unsupported AOI shape '{shape}' in AOI {idx + 1}. Must be 'rectangle' or 'circle'."
            )
        ...
        if shape == "rectangle":
            if len(coordinates) != 4:
                raise ValueError(
                    f"Rectangle AOI {idx + 1} must have 4 coordinates: [x1, x2, y1, y2]."
                )
            ...
            if x2 > screen_width or y2 > screen_height:
                raise ValueError(
                    f"AOI {idx + 1} exceeds screen boundaries (Width: {screen_width}, Height: {screen_height})."
                )
            ...
            if len(coordinates) != 3:
                raise ValueError(
                    f"Circle AOI {idx + 1} must have 3 coordinates: [x_center, y_center, radius]."
                )
            ...
            if (
                ...
                raise ValueError(
                    f"Circle AOI {idx + 1} exceeds screen boundaries (Width: {screen_width}, Height: {screen_height})."
                )
...
def plot_heatmap(data, screen_coords, aoi_boundaries=None, bins=None):
    ...
    data = data[
        (data["xpos"] >= 0)
        & (data["xpos"] <= screen_width)
        & (data["ypos"] >= 0)
        & (data["ypos"] <= screen_height)
    ]
    ...
    if bins is None:  # Default bins, 20 px bins here if nothing else is given
        ...
        isinstance(bins, (tuple, list)) and len(bins) == 2
    ):  # User-defined, if different bins for x and y are desired
        bins_x, bins_y = bins
    else:
        raise ValueError("`bins` must be an integer or a tuple of two integers.")

    heatmap = np.histogram2d(data["xpos"], data["ypos"], bins=[bins_x, bins_y])[0]
